...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    ...
    while cap.isOpened():
        ...
        with open(f'save_json/hot_gt.mp4/{i+compare_frame:0>4}.json') as json_file:
            hot_gt_json = json.load(json_file)
        gt_10 = hot_gt.extract_vec_norm_by_part(hot_gt_json)
        if del_true: json_archi=json_archi[1:]
        json_archi.append(hot_gt_json)
        
        
        s = 0
        for part in range(5):
            temp = metric.l2_normalize(gt_10[part], prac[part])#*metric.cosine_similar(gt[part], prac[part])
            if len(total[part])>=20:
                total[part]=total[part][1:] 
                total[5]=total[5][1:]
        ...
        speed_metric = []
        for part in range(6):
            match_point = np.argmin(total[part])
            if match_point-compare_frame+5>0: speed_metric.append("fast")
            elif match_point-compare_frame+5<0: speed_metric.append("slow")
            else : speed_metric.append("good")
        ll=len(json_archi)//2
        # gt = hot_gt.extract_vec_norm_by_part(json_archi[ll])
        
        # print(gt)
        prac_image = hot_prac.visual(keypoints)
        gt_image = hot_gt.visual(json_archi[ll])
